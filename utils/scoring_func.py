...
class SimilarityWithTrain:
    ...
    def get_top_sims(self, mol, top=1):
        similarities = self.get_similarity(mol)
        avg_sim = np.mean(similarities) # Diversity
        idx_sort = np.argsort(similarities)[::-1]
        top_scores = similarities[idx_sort[:top]] # Sim.Train
        top_smiles = self.train_uni_smiles[idx_sort[:top]]
        return avg_sim, top_scores, top_smiles
