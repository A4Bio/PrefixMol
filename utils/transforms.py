...
class RefineData(object):
    ...
    def __call__(self, data):
        ...
        protein_element = data.protein_element # [409]
...
class LigandRandomMask(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        # context ligand atom element/full features/pos. Note: num_neigh and num_valence features should be changed
        ...
        data.ligand_context_num_neighbors = LigandCountNeighbors.count_neighbors(
            ...
            symmetry=True,
            num_nodes = context_idx.size(0),#context_idx.size(0)=23
        )
...
class AtomComposer(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        data.compose_pos = torch.cat([ligand_context_pos, protein_pos], dim=0)#413+18=431
    ...
    def get_knn_graph(data:ProteinLigandData, knn, len_ligand_ctx, len_compose, num_workers=1, ):
        data.compose_knn_edge_index = knn_graph(data.compose_pos, knn, flow='target_to_source', num_workers=num_workers)#[2, 20688]
...
class FocalBuilder(object): #select focal atoms from compose
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if has_unmask_atoms:
            ...
            ind_edge_index_candidate = [ #len(ind_edge_index_candidate)=58
            ...
            index_changer_context = torch.zeros(context_idx.max()+1, dtype=torch.int64)#[27]
            ...
            idx_focal_in_compose = idx_focal_in_ligand_context  # if ligand_context was not before protein in the compose, this was not correct —— Okay
...
class EdgeSample(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        # candidate edge: mask-context edge
        idx_edge_index_candidate = [#len(idx_edge_index_candidate)=64
            (context_node in context_idx) and (mask_node in masked_idx)
            for mask_node, context_node in zip(*old_bond_index)
        ]  # the mask-context order is right
        candidate_bond_index = old_bond_index[:, idx_edge_index_candidate]#[2, 20]
        candidate_bond_types = old_bond_types[idx_edge_index_candidate]#[20]
        
        # index changer
        index_changer_masked = torch.zeros(masked_idx.max()+1, dtype=torch.int64)#[28]
        index_changer_masked[masked_idx] = torch.arange(len(masked_idx))#[15]

        has_unmask_atoms = context_idx.nelement() > 0
        if has_unmask_atoms:
            index_changer_context = torch.zeros(context_idx.max()+1, dtype=torch.int64)#[31]
            index_changer_context[context_idx] = torch.arange(len(context_idx))#[16]
            ...
            if neg_version == 1:  # radiu + tri_edge
                ...
                #negative edge index & types
                false_edge_index = edge_index_knn[:, ~idx_real_edge_index]#[2, 60]
                false_edge_types = torch.zeros(len(false_edge_index[0]), dtype=torch.int64)#[60]


            # cat (# new edge index (positive) & #negative edge index & types)
            # print('Num of pos : neg edge:', len(new_edge_type), len(false_edge_types), len(new_edge_type) / len(false_edge_types))
            new_edge_index = torch.cat([new_edge_index, false_edge_index], dim=-1)#[2,80]
            new_edge_type = torch.cat([new_edge_type, false_edge_types], dim=0)#[80]
...
if __name__ == '__main__':
    ...
    cfg = DefaultMunch.fromDict({'type': 'mixed', 'min_ratio': 0.0, 'max_ratio': 1.1, 'min_num_masked': 1, 'min_num_unmasked': 0, 'p_random': 0.15, 'p_bfs': 0.6, 'p_invbfs': 0.25})
    masking = get_mask(cfg)
    
    composer = AtomComposer(protein_featurizer.feature_dim, ligand_featurizer.feature_dim, 48)
    edge_sampler = EdgeSample(DefaultMunch.fromDict({"k":8}))
    
    cfg_ctr = DefaultMunch.fromDict({'num_real': 20, 'num_fake': 20, 'pos_real_std': 0.05, 'pos_fake_std': 2.0})
    contrastive_sampler = ContrastiveSample(cfg_ctr.num_real, cfg_ctr.num_fake, cfg_ctr.pos_real_std, cfg_ctr.pos_fake_std, 32)
    ...
    config = DefaultMunch.fromDict({'name': 'pl', 'path': '/huyuqi/MolDesign/data/crossdocked_pocket10', 'split': '/huyuqi/MolDesign/data/split_by_name.pt'})
    dataset, subsets = get_dataset(
        config = config,
        transform = transform,
    )
    
    for data in subsets['test']:
        print(data)