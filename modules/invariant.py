...
class MessageModule(Module):
    def __init__(self, node_sca, node_vec, edge_sca, edge_vec, out_sca, out_vec, cutoff=10.):
        super().__init__()
        hid_sca, hid_vec = edge_sca, edge_vec#64,64
        self.cutoff = cutoff
        ...
        self.sca_linear = Linear(hid_sca, out_sca)  # edge_sca for y_sca; the scalar messages(the product of scalar feat of both vertices and edges)
        # the vector messages (the summation of vector feat of vertices and edges after the multiplication with scalar feat)
...
class GVPerceptronVN(Module):#Gper，add a vector nonlinear activation to the output of GVP
...
class GVLinear(Module):#Glin, a geometric vector linear(GVL) block, drop the nonlinear activations of both scalar and vector outputs of GVP
    def __init__(self, in_scalar, in_vector, out_scalar, out_vector):
        super().__init__()
...
class VNLinear(nn.Module):#VNLinear:VectorNonLinear
